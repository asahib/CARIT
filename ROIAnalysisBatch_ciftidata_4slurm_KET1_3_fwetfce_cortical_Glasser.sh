...
Subjlist=$(<$maindir/logstxt/Sublist.txt) #Space delimited list of subject IDs
...
templatesdir="${maindir_jl}/scripts/ParcellationTemplates/md2242_pkk1V_pkk1V" 
...
FuncMaskname="${groupzstat}_mask_tfce_glasser_${corrp}${pvalue}"
mkdir ${FuncMasksdir}
...
FuncMasks="YES"
MakeMasks="YES" 
#YES if we want to isolate freesurfer masks
...
wb_command -cifti-create-dense-from-template ${templatesdir}/Glasser_NetworkPartition_v9.dlabel.nii ${FuncMasksdir}/${FuncMaskname}.dscalar.nii -cifti ${FuncMasksdir}/${FuncMaskname}.dscalar.nii
fi
...
then 
	...
	do
		echo "Generating masks from Glasser for ${sub}"
		...
		if [ ${SubcorticalROI} = "YES" ]; then
				...
				do
					...
					wb_command -cifti-label-to-roi ${templatesdir}/Glasser_NetworkPartition_v9.dlabel.nii ${freesurferMasksdir}/cifti_${mask}_aparcaseg.dscalar.nii -name ${mask}
				...
				for mask in $ROIlist
				do
					echo "creating freesurfer mask ${mask} with name: ${freesurferMasksdir}/cifti_${mask}_aparcaseg.dscalar.nii "
					
					wb_command -cifti-label-to-roi ${templatesdir}/Glasser_NetworkPartition_v9.dlabel.nii ${freesurferMasksdir}/cifti_${mask}_aparcaseg.dscalar.nii -name ${mask}
					echo "creating combined mask with with name: ${CombinedMasksdir}/combined_${mask}_${FuncMaskname}.dscalar.nii"
...
#done
rm ${logdir}/MERGEDROIs_${FuncMaskname}.txt
for sub in ${Subjlist}
do
echo "extracting cope values for sub ${sub}"
	...
	then
		...
		if [ ${MergeROIs} = "YES" ]
		then
			wb_command -cifti-stats ${imagesdir}/cope6.dtseries.nii -reduce MEAN -roi ${CombinedMasksdir}/MERGEDROIs_${FuncMaskname}.dscalar.nii >> ${logdir}/MERGEDROIs_${FuncMaskname}.txt
	
		else
			wb_command -cifti-stats ${imagesdir}/cope6.dtseries.nii -reduce MEAN -roi ${CombinedMasksdir}/combined_${ROIlist}_${FuncMaskname}.dscalar.nii >> ${logdir}/${ROIlist}_${FuncMaskname}.txt
